...
{
	system("cls");
	cout << "\t\t\t_$$$$___$$$$$____$$$$______$$$$$____$$$$___$$$$$$__$$$$$$__$$______$$$$$" << endl;
	cout << "\t\t\t$$______$$______$$__$$_____$$__$$__$$__$$____$$______$$____$$______$$" << endl;
	cout << "\t\t\t_$$$$___$$$$____$$$$$$_____$$$$$___$$$$$$____$$______$$____$$______$$$$" << endl;
	cout << "\t\t\t____$$__$$______$$__$$_____$$__$$__$$__$$____$$______$$____$$______$$" << endl;
	cout << "\t\t\t_$$$$___$$$$$___$$__$$_____$$$$$___$$__$$____$$______$$____$$$$$$__$$$$$";
	cout << endl << endl;
	printf("\t\t\t\t\t\t\t%s Играть\n", iItem == 1 ? ">" : " ");
	printf("\t\t\t\t\t\t\t%s Правила\n", iItem == 2 ? ">" : " ");
	printf("\t\t\t\t\t\t\t%s Авторы\n", iItem == 3 ? ">" : " ");
...
void Rules() {
	system("cls");
	cout << " На поле 10 х 10 спрятаны 4 однопалубных, 3 двухпалубных, 2 трёхпалубных, 1 четверопалубный корабль." << endl;
	cout << "Ваша задача - как можно быстрее уничтожить флот соперника, пока он не уничтожил ваш." << endl;
	int iItem = 2;
	int ch = _getch();
	if (ch == 13) { ShowMenu(iItem); }
};
void Author() {
	system("cls");
	cout << "Авторы:\nРуслан Керимов\nИз группы 1519-1 Дмитровского института непрерывного образования";
	int iItem = 3;
	int ch = _getch();
	if (ch == 13) { ShowMenu(iItem); }
};
...
int main()
{
	...
	while (TRUE)
	{
		ch = _getch();
		switch (ch) {
		case 72: if (0 < iItem - 1) iItem = iItem - 1;
			   else iItem = nLast;
			ShowMenu(iItem);
			break;

		case 80: if (iItem < nLast) iItem = iItem + 1;
			   else iItem = 1;
			ShowMenu(iItem);
			break;

		case 13: switch (iItem) {
		...
		case 1:
			...
			while (chek)
			{
				...
				cout << "Расставите корабли автоматически или вручную? 1-да, 0-нет ";
